...
for subjnumber=subjects     
    ...
    allPeakLoc = [];
    parfor ii = 1:92
        [pwel,fwel] = pwelch(dtrendHR(:,ii),17700 , [], [], 50, 'onesided');
...
xlim([5.5 14.5]);
xlabel("Age (years)",'FontSize',20,'FontWeight','bold',"FontName","Times")
ylabel("Heart Rate (Hz)",'FontSize',20,'FontWeight','bold',"FontName","Times")
...
ylabel("HR (Hz)","FontName","Times",'FontWeight','bold')
...
histogram(mData*60,[70:1:97])
hold on
histogram(fData*60,[70:1:97])
hold off
legend("Male","Female","FontName","Times",'FontWeight','bold')

xlabel("Heart Rate (BPM)",'FontSize',20,"FontName","Times",'FontWeight','bold')
...
ylabel("HR (Hz)",'FontSize',20,"FontName","Times",'FontWeight','bold')
